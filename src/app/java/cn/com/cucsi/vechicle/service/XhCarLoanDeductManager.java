...
//默认将类中的所有函数纳入事务管理.
@Transactional
public class XhCarLoanDeductManager {
	//private XhCarloan
	private XhCarLoanContractDao xhCarLoanContractDao;
	@Autowired
	public void setXhCarLoanDeductDao(XhCarLoanContractDao xhCarLoanContractDao) {
	...
	public void saveXhCarLoanContract(XhCarLoanContract entity) {
		if(entity.getState()!=0){
			...
			//保存到制作表
			entity.getXhCarLoanApply().setState("51");
			xhCarLoanApplyDao.save(entity.getXhCarLoanApply());
			System.out.println("xhCarLoanApplyDao.save(entity.getXhCarLoanApply())");
		}
	...
	//划扣
	public void saveXhCarLoanDeductHuaKou(Date date,MiddleMan middleMan,XhCarLoanContract xhCarLoanContract) {
		//存借款申请表
				...
				xhJksq.setState(CarState.YI_HUA_HOU.getText());
				...
				xhCarLoanApplyManager.saveHistory(oldState, xhApplyOracle, CarOperation.YI_HUA_HOU, CarOperation.YI_HUA_HOU.getMessage());
	}
	 ...
	 */
	@Transactional(readOnly = true)
	public List<Map<String,Object>> searchXhCarLoanHuaKouContract(String queryName,Map<String,Object> filter,JdbcPage page)
	{
		String sql = "";
		String value="";
		if(filter.containsKey("userName")){
			...
			if(StringUtils.isNotEmpty(value)) {
				sql = sql + " and u.user_name like '%" +  value + "%'";
			}
		}
		...
		if(filter.containsKey("makeLoanDate")){
			...
			if(StringUtils.isNotEmpty(value)) {
				sql = sql + " and m.make_loan_date = to_date('" +  value + "','yyyy-MM-dd')";
			}
		}
