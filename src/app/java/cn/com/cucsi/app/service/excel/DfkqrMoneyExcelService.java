...
//默认将类中的所有函数纳入事务管理.
@Transactional
public class DfkqrMoneyExcelService extends AbstractExcelService{
    
    private static Logger logger = LoggerFactory.getLogger(DfkqrMoneyExcelService.class);
    ...
    private void generateSql(Map<String, Object> filter,JdbcPage page){
        ...
        if (filter.containsKey("backup")) {// 证件号码
            if (StringUtils.isNotEmpty(String.valueOf(filter.get("backup")))) {
                sqlBuffer.append(" AND A.BACKUP01 = :backup");
            }
        }
