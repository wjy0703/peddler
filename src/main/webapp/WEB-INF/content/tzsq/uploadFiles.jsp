...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#uploadify09").uploadify({
			'removeCompleted' : false,
			'swf' : 'uploadify/threetwo/uploadify.swf',
			'uploader' : 'xhTzsq/upLoadFile?id=${Id}',
			 buttonText :'',
			...
			'onQueueComplete': function(file,data){ //所有文件上传成功后触发的方法。  
   				if($(successUploadFile).not(filesUpload).length == 0 && $(filesUpload).not(successUploadFile).length == 0){
	   					$.ajax({
					     type: 'POST',
					     url: 'xhTzsq/upLoadFileBack?id=${Id}',
...
<div class="pageContent">
	<div class="pageFormContent">
		...
		<div class="successUploaded">
			<input type="file" name="upload" id="uploadify09" />
		</div>
		...
		<p  style="width:100%">
			...
			<a class="successUploaded" href="javascript:$('#uploadify09').uploadify('cancel','*')"><img
