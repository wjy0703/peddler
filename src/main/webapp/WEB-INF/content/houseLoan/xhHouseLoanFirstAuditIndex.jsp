		...
		<tbody>
			<c:forEach items="${page.result}" var="user" varStatus="st">
				...
				<td>${user.thirdLxrTelphone}</td> -->
					...
					<td><c:if test='${user.loanState=="31"}'>
							<a title="录入初审" target="navTab"
								href="${ctx }/xhHouseLoanAudit/addXhHouseLoanAudit/${user.id}"
								class="">录入初审</a>

						</c:if> <c:if test='${user.loanState=="33"}'>
							<a title="录入合同数据" target="navTab"
								href="${ctx }/xhHouseLoanAudit/addXhHouseLoanAuditInfo/${user.id}"
								class="">录入合同数据</a>
					</c:if></td>
				</tr>
			</c:forEach>
