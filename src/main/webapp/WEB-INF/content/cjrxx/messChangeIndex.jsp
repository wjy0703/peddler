...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/cjrxx/listMessChangeCjr" method="post">
			...
			<table>
				<tr>
					...
					<td><label>CRM城市：</label> <select class="combox"
						name="filter_crmprovince" ref="combox_MessChangeCjrcrmcity"
						...
						id="combox_MessChangeCjrcrmcity">
				...
				<tr>
					<td><label>所在城市：</label> <select class="combox"
						name="filter_province" ref="combox_MessChangeCjrcity"
						refUrl="${ctx}/cjrxx/getCity?code={value}">
			...
			<c:forEach items="${listCjrxx}" var="user" varStatus="st">
				<tr target="sid_user" rel="${user.ID}">
					...
					<a title="查看" target="navTab"
						...
						</c:if> <c:if test="${user.UPSTATE=='0'}">
							<a title="提交待审批" target="ajaxTodo"
								href="${ctx }/cjrxx/subMessChange/${user.ID}">提交</a>
						</c:if></td>
				</tr>
			</c:forEach>
		</tbody>
