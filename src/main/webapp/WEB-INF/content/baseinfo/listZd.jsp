...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/baseinfo/listZd" method="post">
		<div class="searchBar">
			<ul class="searchContent">
				<li><label>字典名称:</label> <input type="text" name="filter_name" />
				</li>
				<li><label>字典状态:</label> <select class="combox"
					name="filter_state">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="${ctx}/baseinfo/addZp" target="navTab"><span>添加</span></a></li>
			<li><a class="edit" href="${ctx}/baseinfo/editZd/{sid_zd}"
	...
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="100" orderField="name" class="asc">字典名称</th>
				<th width="100" orderField="value">字典参数</th>
		...
		<tbody>
			<c:forEach items="${page.result}" var="zd" varStatus="st">
				<tr target="sid_zd" rel="${zd.id}">
					...
					<td><c:if test="${zd.state =='0'}">在用</c:if> <c:if
							test="${zd.state=='1'}">历史</c:if></td>
					<td>${zd.bankCode }</td>
					<td><a title="删除" target="ajaxTodo"
						href="${ctx }/baseinfo/delZd/${zd.id}" class="btnDel">删除</a> <a
						title="编辑" target="navTab" href="${ctx }/baseinfo/editZd/${zd.id}"
