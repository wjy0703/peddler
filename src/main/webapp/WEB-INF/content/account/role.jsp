...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/account/listrole" method="post">
		<div class="searchBar">
			<ul class="searchContent">
				<li><label>角色编码:</label> <input type="text" name="filter_name" />
				</li>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="${ctx}/account/addrole" target="navTab"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids"
				postType="string" href="${ctx }/account/batchdelrole" class="delete"
				warn="请至少选择一个用户"><span>批量删除</span></a></li>
			<li><a class="edit" href="${ctx}/account/editrole/{sid_role}"
	...
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="80" orderField="name" class="asc">角色名称</th>
				<th orderField="cname">角色描述</th>
		...
		<tbody>
			<c:forEach items="${page.result}" var="role" varStatus="st">
				<tr target="sid_role" rel="${role.id}">
					...
					<td>${role.name }</td>
					<td>${role.cname }</td>
					<td><c:if test="${role.sts=='0'}">在用</c:if> <c:if
							test="${role.sts=='1'}">历史</c:if></td>
					<td>${role.createBy }</td>
					<td>${role.createTime }</td>
					<td>${role.lastModifyBy }</td>
					<td>${role.lastModifyTime }</td>
					<td><a title="删除" target="ajaxTodo"
						href="${ctx }/account/delrole/${role.id}" class="btnDel">删除</a> <a
						title="编辑" target="navTab"
						href="${ctx }/account/editrole/${role.id}" class="btnEdit">编辑</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
