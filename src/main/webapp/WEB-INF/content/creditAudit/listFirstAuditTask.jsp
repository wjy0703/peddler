...
<script type="text/javascript">
	function firstUploadFile(param) {
...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/loan/listLoanApplyByState/31" method="post">
		<div class="searchBar">
			<table width="100%">
				<tr>
					...
					<td><label>产品:</label>
					<sen:select  clazz="combox" name="filter_jkType" coding="productType" value="${filter_jkType}" title="全部" titleValue="all"/>
					</td>
				</tr>
				<tr>
		...
		<tbody>
			<c:forEach items="${listJksq }" var="jksq" varStatus="st">
				<tr target="sid_jksq" rel="${jksq.ID}">
					...
					<td>${jksq.TEAMNAME }</td>
					<td>${jksq.SALENAME }</td>
					<td>${fn:substring(jksq.BACKUP02,0,19)}</td>
				...
				<a id="freshJksp" href="${ctx }/xhNewJksq/lookXhJksq/${jksq.ID}?look=1" target="navTab" 
					...
					| <a href="${ctx}/loan/downLoadFile/${jksq.ID},sx:wf"
									target="dialog" title="下载材料" mask="true" width="600"
									height="420">下载</a> 
							
							<c:if test="${jksq.STATE=='31' or jksq.STATE=='32#B'}">	
								| <a title="外访说明" target="navTab"
									href="${ctx}/jksqSurvey/addSurvey/${jksq.ID}">外访</a> 
								
							</c:if>
							
							<c:if test="${jksq.STATE=='31.C'}">
							   <c:if test="${jksq.survey=='1'}">
								| <a title="外访反馈" target="navTab"
									href="${ctx}/jksqSurvey/lookSurveyBackLook/${jksq.ID}">外访反馈</a> 
     							</c:if>
						    </c:if>
							|
							<a href="${ctx }/xhBorrowscore/jksqAuditGrade/${jksq.ID}"
									target="dialog" title="借款评分信息" mask="true" width="900"
									height="620">评分</a> 
									
							
								| <a href="${ctx}/loan/uploadFile/${jksq.ID},1" 
									target="dialog" title="上传信用初审材料" mask="true" width="600"
									height="420" close="firstUploadFile" rel="rel_firstUploadFile"
									param='{"statusCode":"200", "message":"操作成功", "navTabId":"rel_listLoanApplyByState", 

"forwardUrl":"rel_listLoanApplyByState", "callbackType":""}'>上传</a> 
								| <a title="录入初审结果"
									href="${ctx}/loan/addFirstAuditResult/${jksq.ID}"
									target="navTab">初审</a>
									
					 <bjdv:validateContent type="1" funcId="借款申请-联系人">
					<br>
					<a  href="${ctx }/jksq/jksqLxr/${jksq.ID}" 
						target="dwzExport" targetType="navTab" title="是要导出这些记录吗?" rel="rel_jksqShow" >下载电话调查表</a>
		</bjdv:validateContent> 
		
		| <a title="录入审批意见书"
									href="${ctx}/loan/newInitAuditInfo/${jksq.ID}"
									target="navTab">审批意见书</a>
							</div>
