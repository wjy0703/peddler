...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/loan/listCreditAuditResultByType/1" method="post">
		<div class="searchBar">
			...
			<table  width="100%">
				...
				<tr>
					...
					<td><label>初审结果:</label> <select name="filter_creditResult" class="combox">
		...
		<tbody>
			<c:forEach items="${page.result}" var="user" varStatus="st">
				<tr target="sid_user" rel="${user.id}">
					<td><input name="ids" value="${user.loanApply.id}" type="checkbox"></td>
					...
					<td>
					<fmt:formatNumber
								value="${user.loanApply.jkLoanQuota}" pattern="#,#00.00" />
					...
					<td><fmt:formatNumber
								value="${user.outVisitFee}" pattern="#,#00.00" /></td>
...
<div class="buttonActive">
					...
					<div  class='OperationsPopUp' style='display: none; position: absolute; text-align: justify; font-size: 12px;height:40px;width:300px;  filter:alpha(opacity=100);
					   ...
					   word-wrap:break-word;
					    line-height:100%;
					'>
					<a href="${ctx }/jksq/jksqHistoryList/${user.loanApply.id}" rel="rel_jksqhis_state"  mask="true" 
				...
				<a id="freshJksp" href="${ctx }/xhNewJksq/lookXhJksq/${user.loanApply.id}?look=1" target="navTab" 
						title="查看借款申请信息" rel="rel_jksqShow">查看申请</a>
				...
				<br>
							...
							<a href="${ctx }/xhBorrowscore/jksqAuditGrade/${user.loanApply.id}?look=1"
									target="dialog" title="借款评分信息" mask="true" width="900"
									height="620">评分结果</a> 		
				
						|<a class="icon"
					href="${ctx}/loan/listFirstAuditHistory/${user.loanApply.id}/1"
					target="dialog" rel="dlg_page2" title="初审结果" width="800"
