...
<div class="panel">
	...
	<div class="pageContent">
		<form method="post" id="jkhtform" name="jkhtform"
			action="${ctx}/xhJkht/saveAuditXhJkhtQzqr"
			...
			<input type="hidden" name="id" value="${xhJkht.id}" /> <input
				...
				<input type="hidden" name="auditPerson" value="${xhJkht.auditPerson}" />
				<input type="hidden" name="auditIdea" value="${xhJkht.auditIdea}" />
			...
			-->
					...
					<tr>
						...
						<td><label>还款付息方式：</label><sen:select clazz="combox" name="rePayType" coding="returnIntest" value="${xhJkht.rePayType}"/></td>
						<td></td>
					</tr>
			<tr>
			<td colspan="3">
				...
				<dl class="nowrap">
					<dt>审核意见：</dt>
					<dd>
						<textarea name="auditQzqrIdea" style="width: 93%; height: 80">${xhJkht.auditQzqrIdea}</textarea>
					</dd>
				</dl>
				<dl class="nowrap">
					<dt>审核结果：</dt>
					<dd>
						<input type="radio" name="state" value="2" />通过 <input
							type="radio" name="state" value="3" />不通过
					</dd>
				</dl>
			</div>
			<div class="formBar">
