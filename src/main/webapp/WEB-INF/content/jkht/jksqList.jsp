...
<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/xhJkht/listJksq/50" method="post">
				<div class="searchBar">
			<table>
				<tr>
					<td><label>客户姓名:</label> 
...
<c:if test="${jksq.XHJKHT_ID == null || jksq.XHJKHT_ID == '' }">
		<div class="buttonActive">
			<div class="buttonContent">
			<a title="审核利率" href="${ctx}/xhJkht/addXhJkht/${jksq.ID}"
					target="navTab" mask="true" >
				<button type="submit">审核利率</button></a>
			</div>
		</div>
	</c:if>
	<c:if test="${jksq.XHJKHT_ID != null}">
		<div class="buttonActive">
			<div class="buttonContent">
			<a title="删除" target="ajaxTodo" href="${ctx }/xhJkht/delXhJkht/${jksq.ID}" target="navTab" mask="true">
			<button type="submit">删除</button></a>
			</div>
		</div>		
		<div class="buttonActive">
			<div class="buttonContent">
			<a title="审核利率" target="navTab" href="${ctx }/xhJkht/editXhJkht/${jksq.ID}" target="navTab" mask="true">
			<button type="submit">审核利率</button></a>
			</div>
		</div>			
	</c:if>				
