...
<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/xhJkht/listJksq/56" method="post">
				<div class="searchBar">
			<table>
				<tr>
					<td><label>客户姓名:</label> 
		...
		<tbody>
			<c:forEach items="${listJksq }" var="jksq" varStatus="st">
				<tr  target="sid_jksq" rel="${jksq.ID}">
					...
					<td>
<!--  	<div class="buttonActive" id="option[${jksq.ID}]" style="display:block">
							<div class="buttonContent">
								
						<button type="button" onMouseOver="showAllOption(${jksq.ID})" )" >操作</button>
							</div>
</div>-->
<!--<div id="allOption[${jksq.ID}]" style="display:block">-->
...
'>
		...
		<div class="buttonActive">
			<div class="buttonContent">
			<a title="审批" target="navTab" href="${ctx }/xhJkht/auditXhJkhtQzqr/${jksq.ID}" target="navTab" mask="true">
			<button type="submit">审批</button></a>
			</div>
		</div>		
		...
		<div class="buttonActive">
			...
			<a href="${ctx }/jksq/jksqHistoryList/${jksq.ID}" rel="rel_jksqhis_state"  mask="true" 
						title="历史状态" lookupGroup="authoritys"><button type="submit">历史</button></a> 
						</div>
		</div>		
		</div>
