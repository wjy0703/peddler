...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/zxgl/listxydkzx" method="post">
		<div class="searchBar">
			<table class="searchContent">
				...
				<tr>
					<td>
					    <label>城市：</label>
					    <sen:address names="filter_crmprovince,filter_crty" titles="所有省市,所有城市"/>
								<%--  <select class="combox"
		...
		<tbody>
			...
			<c:forEach items="${listKhzx}" var="xydkzx" varStatus="st">
				<tr target="sid_xydkzx" rel="${xydkzx.id}">
					...
					<td><c:if test="${xydkzx.ZHUANG_TAI=='0' }">继续跟踪</c:if> <c:if
							test="${xydkzx.ZHUANG_TAI=='1' }">客户放弃</c:if> <c:if
							test="${xydkzx.ZHUANG_TAI=='2' }">不满足进件条件</c:if> <c:if
							test="${xydkzx.ZHUANG_TAI=='3' }">已进件</c:if>
							<c:if
							test="${xydkzx.ZHUANG_TAI=='10' }">循环借</c:if></td>
					<td><c:if
