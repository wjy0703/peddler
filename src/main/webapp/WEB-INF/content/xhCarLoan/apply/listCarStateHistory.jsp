...
<form id="pagerForm" method="post" action="${ctx }/CarState/listCarStateHistory/${carApplyId }" >
...
<div class="pageContent">
	...
	<table class="table" width="100%"  layoutH="75" nowrapTD="false" targetType="dialog">
		...
		<tbody>
			<c:forEach items="${page.result}" var="carstate" varStatus="st">
				<tr  rel="${carstate.id}">
					<td>${st.count}</td>
					<td align="center">
						 <sen:carStateToName value="${carstate.newState}"/>
					</td>
					<td align="center"><fmt:formatDate value="${carstate.createTime}" type="both"  pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td align="center">${carstate.createBy}</td>
					<td align="center">${carstate.remarks}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
