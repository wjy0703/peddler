...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);"
		action="${ctx }/jksq/listChangeHis" method="post">
		<div class="searchBar">
			<table>
				<tr>
					<td><label>客户姓名:</label> 
				...
				<tr>
					...
					<td><label>所属城市:</label> 
						<select name="filter_province" ref="jksqchangeexam_city" class="combox" 
						...
						<select id="jksqchangeexam_city" name="filter_city" class="combox" 
		...
		<tbody>
			<c:forEach items="${jksqChange }" var="jksq" varStatus="st">
				<tr  target="sid_jksq" rel="${jksq.ID}">
					...
					<td>
						<c:if test='${null != jksq.JK_LOAN_QUOTA && jksq.JK_LOAN_QUOTA != 0 }'>￥<fmt:formatNumber value="${jksq.JK_LOAN_QUOTA }" pattern="#,#00.00" /></c:if>
					</td>
					<td>
						<c:if test='${null != jksq.FKJE && jksq.FKJE != 0 }'>￥<fmt:formatNumber value="${jksq.FKJE}" pattern="#,#00.00" /></c:if>
					</td>
					<td>${jksq.JK_CYCLE }</td>
					<td>${jksq.TEAMNAME }</td>
					<td>${jksq.SALENAME }</td>
					...
					<td>
						...
						<div class="buttonActive">
							<div class="buttonContent">
								<a href="${ctx }/jksq/listChangeHis/${jksq.ID}" rel="rel_jksqChangehist"
										target="navTab" title="借款变更历史"><button type="submit">变更历史</button></a>
							</div>
						</div>
					</td>
				</tr>
