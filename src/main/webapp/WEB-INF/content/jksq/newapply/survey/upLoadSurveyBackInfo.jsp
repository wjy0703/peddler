...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#uploadify05").uploadify({
			'removeCompleted' : false,
			'swf' : 'uploadify/threetwo/uploadify.swf',
			'uploader' : '${upUrl}?id=${Id }&jksqId=${jksqId }',
			 buttonText :'',
			...
			'onQueueComplete': function(file,data){ //所有文件上传成功后触发的方法。  
   				...
   				if($(successUploadFile).not(filesUpload).length == 0 && $(filesUpload).not(successUploadFile).length == 0){
   					...
   					$("#fileName",$box).val(successFileInfos.toString());
   					$("#successInfos").html("上传成功");
   				}else{
...
<div class="pageContent">
	<div class="pageFormContent">
		...
		<div class="successUploaded">
			<input type="file" name="upload" id="uploadify05" />
		</div>
		...
		<p  style="width:100%">
			...
			<a class="successUploaded" href="javascript:$('#uploadify05').uploadify('cancel','*')"><img
