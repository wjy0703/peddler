...
import cn.com.cucsi.core.orm.PropertyFilter.MatchType;
...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	...
	public Page<T> findPage(final Page<T> page, final Criterion... criterions) {
 	...
 	/**
 	 * 根据MAP(params)中的条件进行过滤查询，查询结果分页返回
 	 * @param page  分页参数，由controller层传递过来。
 	 * @param params 条件map
 	 * @param criteriaSetup Criteria的回调函数。
 	 * @return
 	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public Page<T> findPage(final Page<T> page, Map<String, Object> params, CriteriaSetup criteriaSetup){
		
		Criteria criteria = getSession().createCriteria(entityClass);
		criteriaSetup.setup(criteria, params);
	...
	/**
	 *  函数作用同{@link #findPage(final Page<T> page, Map<String, Object> params, CriteriaSetup criteriaSetup)}
	 * @param page
	 * @param params
	 * @return
	 */
	public Page<T> findPage(final Page<T> page, Map<String, Object> params){		
		return findPage(page, params, getDefaultCriteriaSetup());		
	}
	 ...
	 * 执行SQL语句
	 */
	public int executeUpdateBySql(final String queryString) {
